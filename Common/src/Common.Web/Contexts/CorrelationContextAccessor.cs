...
namespace Common.Web.Contexts
{
    public class CorrelationContextAccessor : ICorrelationContextAccessor
    {
        ...
        public CorrelationContextAccessor(IHttpContextAccessor httpContextAccessor,IOptions<CorrelationContextOptions> options)
        {
            _httpContextAccessor = httpContextAccessor;
            _options = options.Value;
            _context[_options.CorrelationIdHeaderKey] = httpContextAccessor.HttpContext?.Request.Headers[_options.CorrelationIdHeaderKey] ??
        ...
        public CorrelationContext CorrelationContext
        {
            get
            {
                var contextBody = _httpContextAccessor.HttpContext?.Request.Headers[_options.CorrelationContextHeaderKey];
                if (contextBody is not null)
                    return JsonConvert.DeserializeObject<CorrelationContext>(contextBody);

                return new CorrelationContext
                {
                    CorrelationId = _context[_options.CorrelationIdHeaderKey]
                };
            }
        }
