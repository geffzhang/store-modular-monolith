...
namespace Common.Vault
{
    internal static class Extensions
    {
        ...
        private static void VerifyOptions(VaultOptions options)
        {
            ...
            if (options.Kv.EngineVersion == 0) options.Kv.EngineVersion = 2;
        }
        ...
        private static async Task AddVaultAsync(this IConfigurationBuilder builder, VaultOptions options)
        {
            ...
            if (options.Kv is null || !options.Kv.Enabled) return;

            var kvPath = options.Kv?.Path;
            if (string.IsNullOrWhiteSpace(kvPath)) throw new VaultException("Vault KV secret path can not be empty.");
        ...
        {
            return options.AuthType?.ToLowerInvariant() switch
            {
